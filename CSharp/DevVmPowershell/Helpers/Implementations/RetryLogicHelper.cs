...
namespace Helpers.Implementations
{
	public class RetryLogicHelper : IRetryLogicHelper
	{
		...
		public T RetryFunction<T>(int numberOfRetries, int delayInSeconds, Func<T> operation)
		{
			...
			{
				throw new Exception("Retrying operation with return failed to execute without running into more exceptions on every retry", ex);
			}
		}
		...
		public async Task<T> RetryFunctionAsync<T>(int numberOfRetries, int delayInSeconds, Func<Task<T>> operation)
		{
			...
			{
				throw new Exception("Retrying async operation with return failed to execute without running into more exceptions on every retry", ex);
			}
		}
		...
		public void RetryFunction(int numberOfRetries, int delayInSeconds, Action operation)
		{
			...
			{
				throw new Exception("Retrying operation with no return failed to execute without running into more exceptions on every retry", ex);
			}
		}
		...
		public async Task RetryFunctionAsync(int numberOfRetries, int delayInSeconds, Func<Task> operation)
		{
			...
			{
				throw new Exception("Retrying async operation with no return failed to execute without running into more exceptions on every retry", ex);
			}
		}
