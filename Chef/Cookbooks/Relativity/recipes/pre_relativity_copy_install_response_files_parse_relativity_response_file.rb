...
copy_file_to_vm_from_host 'copy_relativity_response_file' do
  file_source "#{node['relativity']['response_file']['source_folder']}\\#{node['relativity']['response_file']['file_name']}"
  file_destination response_file_path
  file_destination_folder node['relativity']['response_file']['destination_folder']
end

# Read File into String
ruby_block 'Read Relativity Response File' do
  block do
      ...
      f.each_line do |line|
        ...
        if clean_line != "" && clean_line.length > 0 && clean_line[0] != "#"
          ...
          if line_elements.length > 0
            ...
            puts "Relativity Response File - Parsed Value: #{name}"
          end
        end
      end
      node.set['relativity']['response_file']['parsed_values'] = values
    end
  end
end
