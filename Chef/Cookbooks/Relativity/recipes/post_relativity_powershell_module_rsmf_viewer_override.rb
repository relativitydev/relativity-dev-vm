...
powershell_script 'enable_viewer_for_rsmf' do
  code <<-EOH
    ...
    Add-RsmfViewerOverride -RelativityInstanceName #{node['windows']['new_computer_name']} -SqlAdminUserName #{node['sql']['user']['eddsdbo']['login']} -SqlAdminPassword #{node['sql']['user']['sa']['password']}
    EOH
end
