...
powershell_script 'update_instance_setting - RelativityInstanceUrl' do
  ...
  Reset-InstanceSettingValue -RelativityInstanceName #{node['windows']['new_computer_name']} -RelativityAdminUserName #{node['relativity']['admin']['login']} -RelativityAdminPassword #{node['relativity']['admin']['password']} -Name RelativityInstanceURL -Section Relativity.Core -NewValue "http://#{instance_name_upper_case}/Relativity"
  EOH
end
